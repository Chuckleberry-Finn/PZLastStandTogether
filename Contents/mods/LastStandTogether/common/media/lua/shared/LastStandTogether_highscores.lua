...
function highscore.save()
    ...
    local data = {
        topKills = highscore.crowned.topKills or {},
        topWave = highscore.crowned.topWave or {},
    }

    local path = _internal.getSavePath("lastStandTogether", "LastStandTogether_HighScores.json")
    local writer = getFileWriter(path, true, false)
    if not writer then print("[LastStandTogether] ERROR: Could not write "..path) return end
    writer:write(_internal.jsonEncode(data))
    writer:close()
end
...
function highscore.load()
    ...
    if not data then print("[LastStandTogether] Failed to load high scores: "..(err or "")) return end
