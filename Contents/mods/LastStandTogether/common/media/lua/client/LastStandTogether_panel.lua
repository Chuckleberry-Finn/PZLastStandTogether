...
function lastStandTogetherPanel:prerender()
    ...
    self.resetButton:setEnable(self.canModify and (not not zoneDef.center))
    ...
    if not self.canModify then
        self:drawTextCentre("Requires sandbox options permission to modify settings. (Admin)", self.width/2, self.height-(self.fontMedHeight), 0.9, 0.2, 0.2, 1, UIFont.Small)
    end
...
function lastStandTogetherPanel:initialise()
    ...
    local canModify = not isClient() or getPlayer():getRole():hasCapability(Capability.SandboxOptions)
    ...
    self.StartButton:setTooltip("Start the game at the building you are currently inside.")
    self.StartButton:setEnable(canModify)
    ...
    self.StartButtonRandom:setTooltip("Start the game at a randomly selected building on the map.")
    self.StartButtonRandom:setEnable(canModify)
    ...
    self.resetButton:setTooltip("Reset shop marker positions for the current building.")
    self.resetButton:setEnable(canModify)
    ...
    for k,v in pairs(SandboxVars.LastStandTogether) do
        ...
        if not canModify then button:setEnable(false) end
